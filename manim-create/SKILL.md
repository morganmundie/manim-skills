...
3. **Invent a visual worth watching — read `references/creative_direction.md` at this step.** The first visual that comes to mind is almost always a cliché (a static sine curve, a generic bar chart, a labeled-boxes diagram, text fading in). Reject it and design a scene that shows the *mechanism* of this specific concept: pick a concrete instance with real data computed in the scene, find the one quantity that varies (that's your `ValueTracker`), and show it from multiple synced views. For animations about electricity, computer systems, machine learning, or math — the most common subjects — also pull the matching recipe from `references/domain_recipes.md`.
4. **Map concepts to manim vocabulary.** Use `references/vocabulary.md` to translate phrases like "morphs into", "zooms in", "counts up", "draws itself" into the actual class names (`Transform`, `MovingCameraScene`, `ValueTracker`, `Create`, etc.).
5. **Sketch the shape of the scene before writing code**: what mobjects exist, what order they appear/change/leave in, roughly how long each beat takes. A short mental (or literal, in a comment) storyboard prevents a scene that's just a pile of disconnected `self.play()` calls. Every beat should either set up or deliver an "aha" moment — if a beat would work for any topic, it's filler; cut it or make it specific.
6. **Write one `Scene` subclass** (or `ThreeDScene`/`MovingCameraScene` if the idea needs 3D or camera movement — see `references/scene_types.md`) with a `construct()` method containing the animation.
7. **Render a low-quality preview** to sanity-check it (`uv run manim -pql <file>.py <SceneName>`) if you have shell access. If you don't have execution access in this environment, just hand over the code and tell the user the render command to run themselves — don't pretend to have previewed it.
8. **Iterate based on what the user says about the result**, not by guessing what "looks better" in the abstract.
...
- `references/creative_direction.md` — **read while deciding what to show.** How to get past the first cliché idea: ideation questions, and the signature moves of high-craft explainers (one `ValueTracker` driving many synced views, real computation in the scene, flying diagram parts into equations, montages of randomized cases, glow/passing-flash/dim-the-rest effects, camera choreography, subclass-config variants) — plus a manimgl→ManimCE translation table for when 3b1b's published scene code is used as inspiration.
- `references/domain_recipes.md` — concrete scene patterns for the four most common subject areas: **electricity/EM** (traveling waves, 3D E⊥B waves, charge fields with `StreamLines`, circuits from primitives, flowing current), **computer systems** (logic-gate evaluation, memory grids and data movement, pipelines/packets, code execution views), **machine learning** (networks built from real weight matrices, forward/backward pass choreography, real gradient-descent trajectories, attention heatmaps, convolution), and **math** (rearrangement proofs, diagram→equation, number-grid combinatorics, progressive refinement).
